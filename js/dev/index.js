...
function initSliders() {
  if (document.querySelector(".page--result__slider")) {
    let updateCounter2 = function(swiper) {
      const current = swiper.realIndex + 1;
      const total = swiper.slides.length;
      counterEl.textContent = `${current}/${total}`;
    };
    var updateCounter = updateCounter2;
    const sliderEl = document.querySelector(".page--result__slider");
    const counterEl = sliderEl.querySelector(".page--result__counter");
    new Swiper(sliderEl, {
      modules: [Navigation],
      observer: true,
      observeParents: true,
  ...
  if (document.querySelector(".page--team__slider")) {
    const sliderEl = document.querySelector(".page--team__slider");
    new Swiper(sliderEl, {
      modules: [Navigation],
      observer: true,
      observeParents: true,
      speed: 800,
      // Стартові значення — мобільні
      slidesPerView: 1.2,
      spaceBetween: 16,
      navigation: {
  ...
  if (document.querySelector(".page--video__slider")) {
    let updateVideoCounter2 = function(swiper) {
      const current = swiper.realIndex + 1;
      const total = swiper.slides.length;
      const counter = document.querySelector(".page--video__counter");
      if (counter) counter.textContent = `${current}/${total}`;
    };
    var updateVideoCounter = updateVideoCounter2;
    const sliderEl = document.querySelector(".page--video__slider");
    new Swiper(sliderEl, {
      modules: [Navigation],
      observer: true,
      observeParents: true,
      ...
      breakpoints: {
        ...
        992: {
          slidesPerView: 3,
          centeredSlides: false,
          // ❗ змінюємо на false для коректної позиції
          spaceBetween: 24
        },
        1200: {
          slidesPerView: 3,
          centeredSlides: true,
          // ❗ тільки на великих екранах, коли є місце
          spaceBetween: 32
        }
      },
      on: {
        init(swiper) {
          updateVideoCounter2(swiper);
        },
        slideChange(swiper) {
          updateVideoCounter2(swiper);
        }
  ...
  if (document.querySelector(".page--reviews__slider")) {
    const sliderEl = document.querySelector(".page--reviews__slider");
    new Swiper(sliderEl, {
      modules: [Navigation],
      observer: true,
      observeParents: true,
...
document.querySelectorAll("[data-fls-radio]").forEach((group) => {
  const inputs = group.querySelectorAll(".options__input");
  const items = group.querySelectorAll(".options__item");
  inputs.forEach((input) => {
    if (input.checked) {
      input.closest(".options__item").classList.add("active");
    }
  });
  inputs.forEach((input) => {
...
document.addEventListener("DOMContentLoaded", function() {
  const steps = document.querySelectorAll(".page--quiz__step");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const items = Array.from(content.children).filter((el) => el.tagName === "DETAILS");
  const button = wrapper.querySelector(".page--services__more");
  const limit = 6;
  let expanded = false;
  function collapseToLimit() {
    const toHide = items.slice(limit);
    toHide.forEach((item) => {
      item.classList.remove("is-hidden");
      const body = item.querySelector(".page--services__body");
      const label = item.querySelector(".page--services__label");
    ...
    requestAnimationFrame(() => {
      ...
      requestAnimationFrame(() => {
        toHide.forEach((item) => item.classList.add("is-hidden"));
        requestAnimationFrame(() => {
          content.style.maxHeight = content.scrollHeight + "px";
          setTimeout(() => {
  ...
  function expandAll() {
    ...
    toShow.forEach((item) => item.classList.remove("is-hidden"));
    requestAnimationFrame(() => {
      content.style.maxHeight = content.scrollHeight + "px";
      setTimeout(() => {
        content.style.maxHeight = "none";
    ...
    if (form.hasAttribute("data-ajax")) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.error("Помилка надсилання форми:", response.status);
        }
      } catch (error2) {
        console.error("Мережева помилка:", error2);
      }
    } else {
      formValidate.formClean(form);
      window.flsPopup.open("thx");
